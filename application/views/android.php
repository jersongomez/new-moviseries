<?php require_once 'base.php' ?>
<?php startblock('title') ?>Moviseries Android App <?php endblock() ?>
<?php startblock('meta') ?>
<meta name="description" content="Moviseries peliculas y series en HD - descargar aplicacion android">
<?php endblock() ?>


<?php startblock('main-content') ?>
<div class="container">
    <hr>
    <h2 class="text-center">¿Sabias que Moviseries también esta disponible para ANDROID?</h2>
    <hr>
    <p>Nuestra misión es ir mucho más alla de la web por eso traemos para ti nuestra aplicación oficial para
    dispositivos android con la cual podras ver todo el contenido de moviseries desde cualquier lugar.</p>

    <h3>REQUISITOS DE LA APLICACIÓN</h3>
    <ul>
        <li>Un dispositivo con android 4.1 o superior.</li>
        <li>Tener creado una cuenta en moviseries.xyz (free o premium).</li>
    </ul>


    <h3>CONTENIDO DE LA APLICACIÓN</h3>
    <ul>
        <li>Peliculas y series en HD720p, Dvdrip.</li>
        <li>Peliculas y series en HD1080p (solo usuarios premium).</li>
        <li>Enlaces de descarga openload, google drive.</li>
        <li>Enlaces de descarga desde MEGA (solo usuarios premium).</li>
    </ul>
    <hr>

    <div class="text-center">
        <p>Recuerda Moviseries no se responsabiliza por el mal uso que le des al contenido de nuestra web o app.</p><br>
        <a class="btn btn-primary btn-lg active" href="<?php echo base_url('assets/apps/moviseries.apk') ?>"><i class="icon-download"></i> DESCARGAR APLICACIÓN</a>
    </div>
    <hr>

</div>

<?php endblock() ?>
